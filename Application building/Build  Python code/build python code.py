...
#our application
#render_template is used for rendering the html pages from
tensorflow.keras.models import load_model app =
Flask(__name__)#our flask app model =
load_model('crude_oil.h5',)#loading the model in the flask app
@app.route('/')
def home() :
...
print(x_input) for i in range(0,
len(x_input)):
...
lst_output=[] n_steps=10 i=0
while(i<1):
if(len(temp_input)>10):
#print("temp_input",temp_input)
x_input=np.array(temp_input[1:])
print("{} day input {}".format(i,x_input)) 
x_input=x_input.reshape(1,-1) x_input =
...
x_input = (x_input.reshape(1, n_steps,1)) yhat
= model.predict( x_input, verbose=0)
print(yhat[0])
temp_input.extend(yhat[0].tolist())
...
print(lst_output)
return render_template("web.html",showcase = 'The next day predicted value is:'+
str(lst_output))
#print str(x) if __name__
== '__main__' :
